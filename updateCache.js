function updateCache() {
  ...
  if(now > springStart && now < springEnd)
  {
    ...
    if(now >= springStart && now <= springEnd) { cache.put("stopDayDate", springEnd, 25 * 60 * 60); }
    else { cache.put("stopDayDate", null, 25 * 60 * 60); }
  ...
  else if(now > fallStart && now < fallEnd)
  {
    ...
    if(now >= springStart && now <= springEnd) { cache.put("stopDayDate", fallEnd, 25 * 60 * 60); }
    else { cache.put("stopDayDate", null, 25 * 60 * 60); }
